...
// For example, `Geosmin` has the string representation 
// C12H22O, 
// so it has 
// 12 C atoms, 
// 22 H atoms, and 
...
// `arguments` is an 'array-like object' but is not actually an array so we can
// not use the `Array.forEach` method on it.
// we can instead create an array from `arguments` then use `forEach` =>
// let args = Array.from(arguments);