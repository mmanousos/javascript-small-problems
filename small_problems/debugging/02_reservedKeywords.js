...
// the forEach method does not terminate early. So it completes comparing every 
// value in the `RESERVED_KEYWORDS` array to `name` then returns `false`
// we can either use a `for` loop or the `includes` method to check for a match 
// instead
...
function isReserved(name) {
  return RESERVED_KEYWORDS.includes(name); // includes returns a boolean!
}
